"""Kalshi snapshot — minimal, no SDK magic."""
...
def get(path: str, params: dict | None = None):
    ...
    r = httpx.get(full, headers=headers("GET", f"/trade-api/v2{path}"),
                  params=params, timeout=15)
    print(f"GET {path} → {r.status_code}", file=sys.stderr)
    if r.status_code == 401:
        print(f"  401 body: {r.text[:200]}", file=sys.stderr)
        return None
    r.raise_for_status()
    return r.json()
...
lines.append(f"\n## Balance: ${(bal.get('balance',0) if bal else 0)/100:.2f}"
             if bal else "\n## Balance: unavailable (auth failed)")
